...
export const StudentFees: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-extrabold text-slate-800 dark:text-slate-100">Fee Payments</h1>
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden">
        <table className="w-full text-xs">
          <thead><tr className="border-b border-slate-100 dark:border-slate-800">
            ...
            <th className="text-left py-3 px-4 font-bold text-slate-400 uppercase">Due Date</th>
            <th className="text-left py-3 px-4 font-bold text-slate-400 uppercase">Status</th>
            <th className="text-left py-3 px-4 font-bold text-slate-400 uppercase">Action</th>
          </tr></thead>
          <tbody>
            {payments.length === 0 && <tr><td colSpan={5} className="text-center py-8 text-slate-400">No fee records found.</td></tr>}
            {payments.map(p => (
              <tr key={p.id} className="border-b border-slate-50 dark:border-slate-900">
                <td className="py-3 px-4 font-semibold text-slate-700 dark:text-slate-200">{p.type} Fee</td>
                <td className="py-3 px-4 font-bold">₹{p.amount.toLocaleString('en-IN')}</td>
                <td className="py-3 px-4 text-slate-400">{new Date(p.dueDate).toLocaleDateString()}</td>
                ...
                <td className="py-3 px-4">
                  {p.status === 'PENDING' ? (
                    <button onClick={() => handlePay(p.id)} disabled={payingId === p.id} className="px-3 py-1.5 bg-teal-600 text-white rounded-lg text-[10px] font-bold hover:bg-teal-700 disabled:opacity-50">
