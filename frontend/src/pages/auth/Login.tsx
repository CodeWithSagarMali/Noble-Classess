...
export const Login: React.FC = () => {
  ...
  return (
    <div className="relative min-h-[85vh] flex items-center justify-center px-4 py-12 bg-surface-1 overflow-hidden transition-colors">
      {/* Brand glow background — matches home hero */}
      <div className="absolute top-1/4 left-1/4 w-80 h-80 rounded-full bg-brand-rose/15 blur-[110px] pointer-events-none -z-0" />
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="p-8 rounded-3xl space-y-6 border border-white/8 bg-surface-1/90 backdrop-blur-xl shadow-premium">
          ...
          <div className="text-center space-y-3">
            ...
            <p className="text-xs text-white/50">Sign in to your student, teacher, or admin portal</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {roles.map((r) => (
              <div
                key={r.label}
                className="flex flex-col items-center gap-1 py-3 rounded-xl border border-white/8 bg-surface-2"
              >
                <r.icon className="w-4 h-4 text-brand-rose" />
                <span className="text-[11px] font-bold text-white/90">{r.label}</span>
              </div>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-surface-2 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-white/30 focus:border-brand-rose focus:ring-2 focus:ring-brand-rose/20 outline-none transition-all"
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                ...
                <Link to="/auth/forgot-password" className="text-[10px] font-bold text-brand-rose hover:underline">
                  Forgot password?
                </Link>
              </div>
              <div className="relative">
          ...
          <div className="text-center pt-2">
            <p className="text-xs text-white/50">
              ...
              <Link to="/auth/register" className="font-bold text-brand-rose hover:underline">
                Create an account
              </Link>
            </p>
          </div>
