...
export const AdminDashboard: React.FC = () => {
  ...
  const cardDefs = [
    { label: 'Enrolled Students', value: stats?.cards.totalStudents ?? 0, icon: Users, color: 'text-brand-rose-light', bg: 'bg-brand-rose/10' },
    { label: 'Faculty Members', value: stats?.cards.totalTeachers ?? 0, icon: UserPlus, color: 'text-brand-rose', bg: 'bg-brand-rose/10' },
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-white/60 mt-1">Comprehensive system overview and management</p>
        </div>
        <button onClick={handleBackup}
          className="flex items-center gap-2 px-5 py-2.5 border border-white/8 rounded-xl text-sm font-bold hover:bg-white/5 transition-colors">
          <FileText className="w-4 h-4 text-brand-rose-light" /> Export Backup
        </button>
      </div>
        ...
        {TABS.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-4 py-2.5 text-xs font-bold capitalize rounded-t-xl transition-colors ${
              activeTab === tab ? 'bg-surface-1 border border-b-0 border-white/8 text-brand-rose' : 'text-slate-400 hover:text-white/80'
            }`}>
            {tab}
          </button>
        ))}
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-surface-1 border border-white/8 rounded-2xl p-6 space-y-4">
            <h2 className="font-bold text-white text-sm">Revenue Breakdown</h2>
          ...
          <div className="bg-surface-1 border border-white/8 rounded-2xl p-6 space-y-4">
            <h2 className="font-bold text-white text-sm">Batches per Course</h2>
            <ResponsiveContainer width="100%" height={200}>
          ...
          {/* Recent Students */}
          <div className="bg-surface-1 border border-white/8 rounded-2xl p-6 space-y-4 lg:col-span-2">
            <h2 className="font-bold text-white text-sm">Recent Registrations</h2>
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {(stats?.recentStudents ?? []).map(s => (
                    <tr key={s.id} className="border-b border-white/8 hover:bg-white/5">
                      <td className="py-2.5 pr-4 font-semibold text-white/70">{s.firstName} {s.lastName}</td>
      ...
      {activeTab === 'admissions' && (
        <div className="bg-surface-1 border border-white/8 rounded-2xl p-6 space-y-4">
          <h2 className="font-bold text-white">All Applications</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="border-b border-white/8">
              ...
              <tbody>
                {admissions.map(a => (
                  <tr key={a.id} className="border-b border-white/8">
                    <td className="py-3 pr-4 font-semibold text-white/70">{a.firstName} {a.lastName}</td>
      ...
      {activeTab === 'enquiries' && (
        <div className="bg-surface-1 border border-white/8 rounded-2xl p-6 space-y-4">
          <h2 className="font-bold text-white">Contact Enquiries</h2>
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {enquiries.length === 0 ? <p className="text-xs text-slate-400 text-center py-6">No enquiries received yet.</p> :
              enquiries.map(e => (
                <div key={e.id} className="p-4 border border-white/8 rounded-xl space-y-2 hover:bg-white/5">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-sm font-bold text-white/70">{e.name} <span className="text-slate-400 font-normal">· {e.email} · {e.phone}</span></p>
                      <p className="text-xs font-semibold text-brand-rose">{e.subject}</p>
                    </div>
        ...
        <div className="bg-surface-1 border border-white/8 rounded-2xl p-6 space-y-4">
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto font-mono text-xs">
            {logs.length === 0 ? <p className="text-center text-slate-400 py-6">No audit records found.</p> :
              logs.map(log => (
                <div key={log.id} className="flex gap-3 py-2 border-b border-white/8">
                  <span className="text-slate-400 shrink-0">{new Date(log.createdAt).toLocaleString()}</span>
                  <span className="bg-brand-rose/10 text-brand-rose px-1.5 rounded shrink-0">{log.action}</span>
                  <span className="text-white/70 truncate">{log.details}</span>
                </div>
              ))
            }
          </div>
